...
check_all_in_document = function(doc)
{
  var c = new Array();
...
check_all_in_document_name = function(doc,name, id)
{
  var c = new Array();
  c = doc.getElementsByTagName('input');
	var value;
...
showBillsFromGraph = function(group){
		new Ajax.Request('/finances/update_income/'+group,
						{asynchronous:true,
						evalScripts:true, 
						onFailure:function(request){alert('There was an error. ')},
						parameters:'authenticity_token=' + encodeURIComponent('13851d6b812765bbc65c8e2beaa467cd23546f75')}); 
		return false;	
}

rent_object_name = function(){
	$('auto_pay_object').value = "Najem(" + $('renter_surname').value + ")";
	return false
}

selectGraphs = function(){
	var graphs = '';
	
	if ($('expected').checked){ graphs += "1"; }
	if ($('real').checked) 		{ graphs += "2"; }
	if ($('net').checked) 		{ graphs += "3"; }
	reloadGraph(graphs)
}

reloadGraph = function(showGraph){
	  tmp = findSWF("chart");
	  x = tmp.reload();
		x = tmp.reload("/statistics/month_graph/"+showGraph);
}
...
dateCheck = function(name1, name2){
	var minYear = 1900;
	var maxYear = 2100;
	var daysInMonths = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
	
	var dateElementId = name1+"_"+name2+"_3i";
	var day = eval("document.getElementById(dateElementId).value")
	dateElementId = name1+"_"+name2+"_2i";
	var month = eval("document.getElementById(dateElementId).value")
	dateElementId = name1+"_"+name2+"_1i";		
	var year = eval("document.getElementById(dateElementId).value")
	
	if (( ((day < 1) || (day > 31)) || ((month < 1)||(month > 12)) || ((year < minYear)||(year > maxYear))) || 
     ( (day > daysInMonths[month-1]) && !((year % 4 == 0)&&(day == 29)&&(month == 2)) )){
		alert('Invalid date');
		return false;
	}else{
		alert("OK");
...
select_tab = function(tab_name){
	var tabs = new Array('rooms_info', 'fittings', 'additionals');
	
	for(var i = 0; i < tabs.length; i++){
		if(tabs[i] === tab_name){
			$(tabs[i]+"_tab").className = "focused_tab";
			$(tabs[i]).show();
		}else{
			$(tabs[i]+"_tab").className = "unfocused_tab";
			$(tabs[i]).hide();
		}
	}
}
...
var hBar = new ItemStyle(63, 3, '', 0, 1, '#000000', '#424755', 'mainButtons', 'mainButtons', 'itemBorderButton', 'itemBorderButton', 
 null, null, 'hand', 'default'); 

var subM = new ItemStyle(22, -2, '&raquo;', -15, 3, '#424755', ' #86A2CA', 'lowText', 'highText', 
...
startMenu('root', false, 15, 16, 20, hBar); 

//************************************************ VZOR PRO HORIZONTÁLNÍ ZAROVNÁNÍ NA STŘED 
//** startMenu('root', false, 'page.winW()/2 - pMenu.menu.root[0].menuW/2+26', 160, 20, hBar); 


//* SEZNAM POLOŽEK 

//*******Hlavní menu 
addItem('&nbsp;Uvod', 'index.php', '', hBar, 69); 
addItem('&nbsp;Scripty', 'mScripty', 'sm:'); 
addItem('&nbsp;Jedna', 'mJedna', 'sm:'); 
addItem('&nbsp;Dva', 'mDva', 'sm:'); 
//*******SubMenu Scripty 
startMenu('mScripty', true, 0, 23, 130, subM); 
addItem('&nbsp;kat1', 'kat1.php', ''); 
addItem('&nbsp;kat2', 'kat2.php', ''); 
addItem('&nbsp;kat3', 'kat3.php', ''); 
addItem('&nbsp;kat4', 'kat4.php', ''); 
addItem('&nbsp;kat5', 'kat5.php', ''); 
addItem('&nbsp;kat6', 'kat6.php', ''); 
addItem('&nbsp;kat7', 'kat7.php', ''); 
//**********SubMenu Jedna 
startMenu('mJedna', true, 0, 23, 90, subM); 
addItem('&nbsp;Jedna1', 'mJedna1', 'sm:'); 
addItem('&nbsp;Jedna2', 'mJedna2', 'sm:'); 
addItem('&nbsp;Jedna3', 'mJedna3', 'sm:'); 
addItem('&nbsp;Jedna4', 'mJedna4', 'sm:'); 
addItem('&nbsp;Jedna5', 'mJedna5', 'sm:'); 
addItem('&nbsp;Jedna', 'jedna.php', ''); 
//**********SubSubMenu Jedna1 
startMenu('mJedna1', true, 92, 0, 140, subM); 
addItem('Jedna1-1', 'jedna.php', ''); 
addItem('Jedna1-2', 'jedna.php', ''); 
addItem('Jedna1-3', 'jedna.php', ''); 
addItem('Jedna1-4', 'jedna.php', ''); 
addItem('Jedna1-5', 'jedna.php', ''); 
//**********SubSubMenu Jedna2 
startMenu('mJedna2', true, 92, 0, 140, subM); 
addItem('Jedna2-1', 'jedna.php', ''); 
addItem('Jedna2-2', 'jedna.php', ''); 
addItem('Jedna2-3', 'jedna.php', ''); 
addItem('Jedna2-4', 'jedna.php', ''); 
addItem('Jedna2-5', 'jedna.php', ''); 
//**********SubSubMenu Jedna3 
startMenu('mJedna3', true, 92, 0, 140, subM); 
addItem('Jedna3-1', 'jedna.php', ''); 
addItem('Jedna3-2', 'jedna.php', ''); 
addItem('Jedna3-3', 'jedna.php', ''); 
addItem('Jedna3-4', 'jedna.php', ''); 
addItem('Jedna3-5', 'jedna.php', ''); 
//**********SubSubMenu Jedna4 
startMenu('mJedna4', true, 92, 0, 140, subM); 
addItem('Jedna4-1', 'jedna.php', ''); 
addItem('Jedna4-2', 'jedna.php', ''); 
addItem('Jedna4-3', 'jedna.php', ''); 
addItem('Jedna4-4', 'jedna.php', ''); 
addItem('Jedna4-5', 'jedna.php', ''); 
//**********SubSubMenu Jedna5 
startMenu('mJedna5', true, 92, 0, 140, subM); 
addItem('Jedna5-1', 'jedna.php', ''); 
addItem('Jedna5-2', 'jedna.php', ''); 
addItem('Jedna5-3', 'jedna.php', ''); 
addItem('Jedna5-4', 'jedna.php', ''); 
addItem('Jedna5-5', 'jedna.php', ''); 
//**********SubMenu Dva 
startMenu('mDva', true, 0, 23, 130, subM); 
addItem('&nbsp;&nbsp;Dva1', 'mDva1', 'sm:', subM, 22, -2, '&laquo;', 2); 
addItem('&nbsp;&nbsp;Dva2', 'mDva2', 'sm:', subM, 22, -2, '&laquo;', 2); 
//**********SubSubMenu Dva1 
startMenu('mDva1', true, -152, 0, 150, subM); 
addItem('Dva1-1', 'dva.php', ''); 
addItem('Dva1-2', 'dva.php', ''); 
//**********SubSubMenu Dva2 
startMenu('mDva2', true, -152, 0, 150, subM); 
addItem('Dva2-1', 'dva.php', ''); 
addItem('Dva2-2', 'dva.php', ''); 
