...
(function() {
		...
		var SERVICE_DISPATCHER = 
		{
			...
			{
				if (parsedUrl.pathParts.indexOf('jaxer-service') == -1) return false; 	// Require jaxer-service in the path
				if (!Jaxer.Extensions.ServiceDispatcher) return false;					// Require the ServiceDispatcher extension to be loaded
				var foundFile = Jaxer.Extensions.ServiceDispatcher.findFile(Jaxer.request.documentRoot, '.js');
				return Boolean(foundFile);												// Require the path to contain a handler file to load
			},
			name: function(parsedUrl)
			{
				var nameIndex = parsedUrl.pathParts.indexOf('jaxer-service') + 1;
				return (parsedUrl.pathParts.length > nameIndex) ? parsedUrl.pathParts[nameIndex] : 'defaultService';
			},
		...
		var RPC_DISPATCHER = 
		{
			urlTest: function(parsedUrl)
			{
				if (parsedUrl.pathParts.indexOf('jaxer-rpc') == -1) return false; 	// Require jaxer-rpc in the path
				if (!Jaxer.Extensions.RPCDispatcher) return false;					// Require the RestDispatcher extension to be loaded
				var foundFile = Jaxer.Extensions.RPCDispatcher.findFile(Jaxer.request.documentRoot, '.js');
				return Boolean(foundFile);											// Require the path to contain a handler file to load
			},
			name: function(parsedUrl)
			{
				var nameIndex = parsedUrl.pathParts.indexOf('jaxer-rpc') + 1;
				return (parsedUrl.pathParts.length > nameIndex) ? parsedUrl.pathParts[nameIndex] : 'defaultRPCService';
			},
			handler: function(resolvedName, parsedUrl)
			{
